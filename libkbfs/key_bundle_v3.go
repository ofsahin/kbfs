...
func writerUDKIMV2ToV3(codec kbfscodec.Codec, udkimV2 UserDeviceKeyInfoMapV2) (
	...
	for uid, dkimV2 := range udkimV2 {
		...
		for kid, info := range dkimV2 {
			// Check for an instance of KBFS-1719,
			// specifically where a reader with a negative
			// offset gets promoted to a writer.
			if info.EPubKeyIndex < 0 {
				return nil, fmt.Errorf(
					"Unexpected negative index for user %s and device %s", uid, kid)
			}
